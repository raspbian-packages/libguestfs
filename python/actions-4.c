...
#ifdef GUESTFS_HAVE_SETFILES
PyObject *
guestfs_int_py_setfiles (PyObject *self, PyObject *args)
{
  PyObject *py_g;
  guestfs_h *g;
  PyObject *py_r = NULL;
  ...
  if (!PyArg_ParseTuple (args, (char *) "OsOO:guestfs_setfiles",
                         &py_g, &specfile, &py_paths, &py_force))
    goto out;
  g = get_handle (py_g);
  paths = guestfs_int_py_get_string_list (py_paths);
  if (!paths) goto out;

#ifdef GUESTFS_SETFILES_FORCE_BITMASK
  if (py_force != Py_None) {
