...
guestfs_int_py_add_domain (PyObject *self, PyObject *args)
{
  ...
  if (py_libvirturi != Py_None) {
    ...
    optargs_s.libvirturi = guestfs_int_py_asstring (py_libvirturi);
  }
#endif
...
PyObject *
guestfs_int_py_cryptsetup_close (PyObject *self, PyObject *args)
{
  PyObject *py_g;
  guestfs_h *g;
  PyObject *py_r = NULL;
  ...
  if (!PyArg_ParseTuple (args, (char *) "Os:guestfs_cryptsetup_close",
                         &py_g, &device))
    goto out;
  g = get_handle (py_g);
...
guestfs_int_py_mkfs_btrfs (PyObject *self, PyObject *args)
{
  ...
  if (py_datatype != Py_None) {
    ...
    optargs_s.datatype = guestfs_int_py_asstring (py_datatype);
  }
#endif
  ...
  if (py_metadata != Py_None) {
    ...
    optargs_s.metadata = guestfs_int_py_asstring (py_metadata);
  }
#endif
...
#ifdef GUESTFS_HAVE_NTFS_CHMOD
PyObject *
guestfs_int_py_ntfs_chmod (PyObject *self, PyObject *args)
{
  PyObject *py_g;
  guestfs_h *g;
  PyObject *py_r = NULL;
  struct guestfs_ntfs_chmod_argv optargs_s;
  struct guestfs_ntfs_chmod_argv *optargs = &optargs_s;
  int r;
  const char *device;
  int mode;
  const char *path;
  PyObject *py_recursive;

  optargs_s.bitmask = 0;

  if (!PyArg_ParseTuple (args, (char *) "OsisO:guestfs_ntfs_chmod",
                         &py_g, &device, &mode, &path, &py_recursive))
    goto out;
  g = get_handle (py_g);

#ifdef GUESTFS_NTFS_CHMOD_RECURSIVE_BITMASK
  if (py_recursive != Py_None) {
    optargs_s.bitmask |= GUESTFS_NTFS_CHMOD_RECURSIVE_BITMASK;
    optargs_s.recursive = PyLong_AsLong (py_recursive);
    if (PyErr_Occurred ()) goto out;
  }
#endif
