...
guestfs_int_ruby_add_drive_ro_with_if (VALUE gv, VALUE filenamev, VALUE ifacev)
{
  ...
  rb_warn ("Guestfs#add_drive_ro_with_if is deprecated; use #add_drive instead");
...
guestfs_int_ruby_get_attach_method (VALUE gv)
{
  ...
  rb_warn ("Guestfs#get_attach_method is deprecated; use #get_backend instead");
...
guestfs_int_ruby_inspect_list_applications (VALUE gv, VALUE rootv)
{
  ...
  rb_warn ("Guestfs#inspect_list_applications is deprecated; use #inspect_list_applications2 instead");
...
/*
 * call-seq:
 *   g.ntfs_chmod(device, mode, path, {optargs...}) -> nil
 *
 ...
 *
 *
 * [Since] Added in version 1.55.13.
 *
 * [Feature] This function depends on the feature +ntfs3g+.  See also {#feature_available}[rdoc-ref:feature_available].
 *
 * [C API] For the C API documentation for this function, see
 *         {guestfs_ntfs_chmod}[http://libguestfs.org/guestfs.3.html#guestfs_ntfs_chmod].
 */
VALUE
guestfs_int_ruby_ntfs_chmod (int argc, VALUE *argv, VALUE gv)
{
  guestfs_h *g;
  TypedData_Get_Struct (gv, guestfs_h, &guestfs_h_data_type, g);
  if (!g)
    rb_raise (rb_eArgError, "%s: used handle after closing it", "ntfs_chmod");

  if (argc < 3 || argc > 4)
    rb_raise (rb_eArgError, "expecting 3 or 4 arguments");

  volatile VALUE devicev = argv[0];
  volatile VALUE modev = argv[1];
  volatile VALUE pathv = argv[2];
  volatile VALUE optargsv = argc > 3 ? argv[3] : rb_hash_new ();
  ...
  volatile VALUE v;
  v = rb_hash_lookup (optargsv, ID2SYM (rb_intern ("recursive")));
  if (v != Qnil) {
    optargs_s.recursive = RTEST (v);
...
guestfs_int_ruby_set_direct (VALUE gv, VALUE directv)
{
  ...
  rb_warn ("Guestfs#set_direct is deprecated; use #internal_get_console_socket instead");
...
guestfs_int_ruby_set_selinux (VALUE gv, VALUE selinuxv)
{
  ...
  rb_warn ("Guestfs#set_selinux is deprecated; use #selinux_relabel instead");
...
guestfs_int_ruby_sfdisk_l (VALUE gv, VALUE devicev)
{
  ...
  rb_warn ("Guestfs#sfdisk_l is deprecated; use #part_list instead");
...
guestfs_int_ruby_zfgrepi (VALUE gv, VALUE patternv, VALUE pathv)
{
  ...
  rb_warn ("Guestfs#zfgrepi is deprecated; use #grep instead");
