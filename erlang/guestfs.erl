...
-export([inspect_get_windows_group_policy/2]).
...
-export([ntfs_chmod/4, ntfs_chmod/5]).
-export([ntfscat_i/4]).
...
inspect_get_windows_group_policy(G, Root) ->
  call_port(G, {inspect_get_windows_group_policy, Root}).
...
ntfs_chmod(G, Device, Mode, Path, Optargs) ->
  call_port(G, {ntfs_chmod, Device, Mode, Path, Optargs}).
...
setfiles(G, Specfile, Paths, Optargs) ->
  call_port(G, {setfiles, Specfile, Paths, Optargs}).
