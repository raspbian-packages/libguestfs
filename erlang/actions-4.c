...
int
run_setfiles (ei_x_buff *retbuff, const char *buff, int *idx)
{
  CLEANUP_FREE char *specfile;
  if (decode_string (buff, idx, &specfile) != 0) return -1;
  CLEANUP_FREE_STRING_LIST char **paths;
  if (decode_string_list (buff, idx, &paths) != 0) return -1;
  ...
  for (int i = 0; i < optargsize; i++) {
    ...
    if (atom_equals (hd_name, "force")) {
      ...
      return unknown_optarg (retbuff, "setfiles", hd_name);
  }
  if (optargsize > 0 && buff[*idx] == ERL_NIL_EXT)
    (*idx)++;
  ...
  if (r == -1)
    return make_error (retbuff, "setfiles");
