...
foreign import ccall unsafe "guestfs.h guestfs_inspect_get_windows_group_policy" c_inspect_get_windows_group_policy
  :: GuestfsP -> CString -> IO CInt

inspect_get_windows_group_policy :: GuestfsH -> String -> IO Bool
inspect_get_windows_group_policy h root = do
  r <- withCString root $ \root -> withForeignPtr h (\p -> c_inspect_get_windows_group_policy p root)
  if (r == -1)
    then do
      err <- last_error h
      fail err
...
foreign import ccall unsafe "guestfs.h guestfs_luks_uuid" c_luks_uuid
  :: GuestfsP -> CString -> IO CString

luks_uuid :: GuestfsH -> String -> IO String
luks_uuid h device = do
  r <- withCString device $ \device -> withForeignPtr h (\p -> c_luks_uuid p device)
  if (r == nullPtr)
    then do
      err <- last_error h
      fail err
