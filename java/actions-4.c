...
JNIEXPORT void JNICALL
Java_com_redhat_et_libguestfs_GuestFS__1setfiles  (JNIEnv *env, jobject obj, jlong jg, jstring jspecfile, jobjectArray jpaths, jlong joptargs_bitmask, jboolean jforce)
{
  guestfs_h *g = (guestfs_h *) (long) jg;
  int r;
  const char *specfile;
  size_t paths_len;
  CLEANUP_FREE char **paths = NULL;
  struct guestfs_setfiles_argv optargs_s;
  const struct guestfs_setfiles_argv *optargs = &optargs_s;
  size_t i;

  specfile = (*env)->GetStringUTFChars (env, jspecfile, NULL);
  paths_len = (*env)->GetArrayLength (env, jpaths);
  paths = malloc (sizeof (char *) * (paths_len+1));
  if (paths == NULL) {
    throw_out_of_memory (env, "malloc");
    goto ret_error;
  }
  for (i = 0; i < paths_len; ++i) {
    jobject o = (*env)->GetObjectArrayElement (env, jpaths, i);
    paths[i] = (char *) (*env)->GetStringUTFChars (env, o, NULL);
  }
  ...
  for (i = 0; i < paths_len; ++i) {
    jobject o = (*env)->GetObjectArrayElement (env, jpaths, i);
    (*env)->ReleaseStringUTFChars (env, o, paths[i]);
  }
