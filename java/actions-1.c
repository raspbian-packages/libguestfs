...
JNIEXPORT void JNICALL
Java_com_redhat_et_libguestfs_GuestFS__1f2fs_1expand  (JNIEnv *env, jobject obj, jlong jg, jstring jdevice)
{
  guestfs_h *g = (guestfs_h *) (long) jg;
  int r;
  const char *device;
...
Java_com_redhat_et_libguestfs_GuestFS__1inspect_1list_1applications2  (JNIEnv *env, jobject obj, jlong jg, jstring jroot)
{
  ...
  for (i = 0; i < r->len; ++i) {
    ...
    fl = (*env)->GetFieldID (env, cl, "app2_class",
                             "Ljava/lang/String;");
    (*env)->SetObjectField (env, jfl, fl,
                            (*env)->NewStringUTF (env, r->val[i].app2_class));
