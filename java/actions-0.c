...
JNIEXPORT void JNICALL
Java_com_redhat_et_libguestfs_GuestFS__1cryptsetup_1close  (JNIEnv *env, jobject obj, jlong jg, jstring jdevice)
{
  guestfs_h *g = (guestfs_h *) (long) jg;
  int r;
  const char *device;
...
Java_com_redhat_et_libguestfs_GuestFS__1inspect_1list_1applications  (JNIEnv *env, jobject obj, jlong jg, jstring jroot)
{
  ...
  jfieldID fl;
  jobject jfl;
  CLEANUP_FREE_APPLICATION_LIST struct guestfs_application_list *r = NULL;
  const char *root;
  size_t i;
...
Java_com_redhat_et_libguestfs_GuestFS__1lstatnslist  (JNIEnv *env, jobject obj, jlong jg, jstring jpath, jobjectArray jnames)
{
  ...
  jfieldID fl;
  jobject jfl;
  CLEANUP_FREE_STATNS_LIST struct guestfs_statns_list *r = NULL;
  const char *path;
  size_t names_len;
...
JNIEXPORT void JNICALL
Java_com_redhat_et_libguestfs_GuestFS__1ntfs_1chmod  (JNIEnv *env, jobject obj, jlong jg, jstring jdevice, jint jmode, jstring jpath, jlong joptargs_bitmask, jboolean jrecursive)
{
  guestfs_h *g = (guestfs_h *) (long) jg;
  int r;
  const char *device;
  ...
  const struct guestfs_ntfs_chmod_argv *optargs = &optargs_s;

  device = (*env)->GetStringUTFChars (env, jdevice, NULL);
  mode = jmode;
  path = (*env)->GetStringUTFChars (env, jpath, NULL);

  optargs_s.recursive = jrecursive;
  optargs_s.bitmask = joptargs_bitmask;
