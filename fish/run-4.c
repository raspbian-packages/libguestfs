...
int
run_findfs_partuuid (const char *cmd, size_t argc, char *argv[])
{
  int ret = RUN_ERROR;
  char *r;
  const char *uuid;
...
int
run_setfiles (const char *cmd, size_t argc, char *argv[])
{
  int ret = RUN_ERROR;
  int r;
  const char *specfile;
  ...
  specfile = argv[i++];
  paths = parse_string_list (argv[i++]);
  if (paths == NULL) goto out_paths;
