...
This command depends on the feature C<acl>.   See also
L</feature-available>.
...
In new code, use the L</add-drive-ro> call instead.
...
drive.  If enabled, operations such as L</fstrim> will be able
...
equivalent to calling L</aug-set> C<expr>, C<val>.
...
This command depends on the feature C<blkdiscard>.   See also
L</feature-available>.
...
discarded by a call to L</blkdiscard> are returned as
...
This command depends on the feature C<blkdiscardzeroes>.   See also
L</feature-available>.
...
This command depends on the feature C<btrfs>.   See also
L</feature-available>.
...
I<This function is deprecated.>
In new code, use the L</btrfs-scrub-full> call instead.
...
C pointer to the handle (ie. L</h *>).  The purpose of this is
...
This command depends on the feature C<linuxcaps>.   See also
L</feature-available>.
...
appliance. Refer to L</set-network>.
...
device with L</cryptsetup-close>.

C<mapname> cannot be C<"control"> because that name is reserved by
device-mapper.

If this block device contains LVM volume groups, then calling
L</lvm-scan> with the C<activate> parameter C<true> will make
them visible.

Use L</list-dm-devices> to list all device mapper devices.

This command depends on the feature C<clevisluks>.   See also
L</feature-available>.
...
This is the same as L</command>, but streams the output
...
calling L</lvm-scan> with the C<activate>
...
may need to call L</add-drive-opts> separately.
...
This command depends on the feature C<sleuthkit>.   See also
L</feature-available>.
...
This command depends on the feature C<extlinux>.   See also
L</feature-available>.
...
This command depends on the feature C<libtsk>.   See also
L</feature-available>.
...
then the returned list from L</find> F</tmp> would be
4 elements:
...
This command depends on the feature C<fstrim>.   See also
L</feature-available>.
...
The errno (see L</last-errno>) will be C<ESRCH> in this
case.
...
find a single backend setting, see L</get-backend-setting>.
...
I<This function is deprecated.>
In new code, use the L</internal-get-console-socket> call instead.
...
I<This function is deprecated.>
In new code, use the L</selinux-relabel> call instead.
...
This is different from L</get-tmpdir>, as we need shorter
paths for sockets (due to the limited buffers of filenames for UNIX
sockets), and L</get-tmpdir> may be too long for them.
...
the temporary directory returned by L</get-tmpdir>.
...
This command depends on the feature C<selinux>.   See also
L</feature-available>.
...
This command depends on the feature C<linuxxattrs>.   See also
L</feature-available>.
...
name (eg. F</dev/sd*>).  Use L</list-devices>,
...
This command depends on the feature C<grub>.   See also
L</feature-available>.
...
This command depends on the feature C<hivex>.   See also
L</feature-available>.
...
I<This function is deprecated.>
In new code, use the L</hivex-value-string> call instead.
...
This command depends on the feature C<inotify>.   See also
L</feature-available>.
...
This command depends on the feature C<journal>.   See also
L</feature-available>.
...
(eg. using L</journal-get>), and C<false> means you
...
This command depends on the feature C<ldm>.   See also
L</feature-available>.
...
which might have included L</mount> and L</umount>, and
...
This is the same as L</statns> except that if C<path>
is a symbolic link, then the link is stat-ed, not the file it
refers to.
...
This call allows you to perform the L</lstatns> operation
on multiple files, where all files are in the directory C<path>.
C<names> is the list of files from this directory.
...
I<This function is deprecated.>
In new code, use the L</cryptsetup-close> call instead.
...
I<This function is deprecated.>
In new code, use the L</cryptsetup-open> call instead.
...
This command depends on the feature C<lvm2>.   See also
L</feature-available>.
...
existing device (such as when the L</luks-open> API
is used).
...
to indicate there was an error doing L</lgetxattr> for this
...
This command depends on the feature C<mdadm>.   See also
L</feature-available>.
...
This command depends on the feature C<linuxfsuuid>.   See also
L</feature-available>.
...
Unlike with L</mknod>, C<mode> B<must> contain only permissions
bits.

The mode actually set is affected by the umask.

This command depends on the feature C<mknod>.   See also
L</feature-available>.
...
This command depends on the feature C<squashfs>.   See also
L</feature-available>.
...
This command depends on the feature C<linuxmodules>.   See also
L</feature-available>.
...
subvolume path (see also L</mountable-subvolume> to
...
subvolume path (see also L</mountable-device> to extract
...
This command depends on the feature C<ntfs3g>.   See also
L</feature-available>.
...
This command depends on the feature C<ntfsprogs>.   See also
L</feature-available>.
...
This command depends on the feature C<gdisk>.   See also
L</feature-available>.
...
This command depends on the feature C<rsync>.   See also
L</feature-available>.
...
This command depends on the feature C<scrub>.   See also
L</feature-available>.
...
This command depends on the feature C<selinuxrelabel>.   See also
L</feature-available>.
...
a single backend setting, see L</set-backend-setting>.
...
See also L</set-program>, L</set-trace>,
L</get-identifier>.
...
This is the same as L</sh>, but streams the output
back, handling the case where the output from the command is
larger than the protocol limit.
...
This command depends on the feature C<syslinux>.   See also
L</feature-available>.
...
This command depends on the feature C<xz>.   See also
L</feature-available>.
...
operation.  (You also need to call L</set-pgroup> so that
...
I<This function is deprecated.>
In new code, use the L</lvm-scan> call instead.
...
This command depends on the feature C<wipefs>.   See also
L</feature-available>.
...
This command depends on the feature C<xfs>.   See also
L</feature-available>.
...
This command depends on the feature C<libyara>.   See also
L</feature-available>.
...
See L</yara-scan> to see how to scan files with the loaded rules.
...
This command depends on the feature C<zerofree>.   See also
L</feature-available>.
