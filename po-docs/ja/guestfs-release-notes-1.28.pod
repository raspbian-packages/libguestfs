...
L<virt-v2v(1)> can import from: VMware vCenter, VMware OVA files, local disk
files, libvirt, RHEL 5 Xen.  It can output to: oVirt or Red Hat Enterprise
...
enables full-featured network connections, with working ICMP, ping and so
on.
...
/usr/share/bash-completion/completions/virt-log, but no _virt_log() function
in that file
