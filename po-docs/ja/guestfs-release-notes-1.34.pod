...
C<os-version> depending on your host system.
...
All OCaml-based tools now use L<getopt_long(3)> for option parsing, and
I<--help> output has been improved (Pino Toscano).
...
I<--firstboot> etc have been renamed to C<guestfs-firstboot> (Pino Toscano).
...
C<mllib.cmxa> library.  All code in F<customize> is now built into a single
...
"Silent rules" are used for OCaml, Java, Erlang and POD.  Use C<make V=1> to
