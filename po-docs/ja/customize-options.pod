...
Inject the Balloon Server (F<blnsvr.exe>) into a Windows guest.  This operation also injects a firstboot script so that the Balloon Server is installed when the guest boots.
