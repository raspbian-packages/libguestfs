...
F</dev/sda>, the second time as F</dev/sdb>, and so on.
...
F<filename> may be the special string C<"/dev/null">.  See L<guestfs(3)/NULL
DISKS>.
...
(such as F</dev/sda>), the drive will also be named
F</dev/disk/guestfs/I<label>>.
...
F</path/to/dest/name>. By default the newly created snapshot is writable, if
the value of optional parameter C<ro> is true, then a readonly snapshot is
...
The C<$PATH> environment variable will contain at least F</usr/bin> and
F</bin>.  If you require a program from another location, you should provide
...
This command compresses F<file> and writes it out to the local file
F<zfile>.
...
L</cryptsetup-open>.  The C<device> parameter must be the name of the
...
Find a backend setting string which is either C<"name"> or begins with
C<"name=">.  If C<"name">, this returns the string C<"1">.  If C<"name=">,
...
I<This function is deprecated.> In new code, use the
L</internal-get-console-socket> call instead.
...
(due to the limited buffers of filenames for UNIX sockets), and
L</get-tmpdir> may be too long for them.
...
(eg. F</dev/sd*>).  Use L</list-devices>, L</list-partitions> etc functions
instead.
...
F</boot/grub/device.map> file first that contains the mapping between grub
...
drive letters (like F<C:\>) to partitions.  This inspection API examines the
...
F<C:\etc\favicon.png> and if it has the correct format, the contents of this
...
F</etc/fstab>.  I<In particular note> that this may return filesystems which
...
L</journal-get>), and C<false> means you have reached the end of the
journal.
...
raw block device and partition names (like F</dev/sda> and F</dev/sda1>).

This returns a hashtable, where keys are the disk labels (I<without> the
F</dev/disk/guestfs> prefix), and the values are the full raw block device
and partition names (eg. F</dev/sda> and F</dev/sda1>).
...
I<This function is deprecated.> In new code, use the L</cryptsetup-close>
call instead.
...
I<This function is deprecated.> In new code, use the L</cryptsetup-open>
call instead.
...
canonical name.  For example, F</dev/mapper/VG-LV> is converted to
F</dev/VG/LV>.
...
(eg. F</dev/sda>), just to a partition.  This appears to be a limitation of
...
(eg. F</dev/sda1>).  Also LVM F</dev/VG/LV>-style names can be used, or
...
L</mountable-subvolume> to extract the subvolume path of the mountable if
any).
...
combination of both the device name and the subvolume path (see also
L</mountable-device> to extract the device of the mountable).
...
C<attributes>. Return an error if the partition table of C<device> isn't
GPT.
...
pass the spec file, usually: C</etc/selinux/> + I<selinuxtype> +
...
string already exists matching C<"name"> or beginning with C<"name=">, then
...
I<This function is deprecated.> In new code, use the L</statns> call
instead.
...
if a non-existent file is specified then it is created with a "hole" before
...
lazy-counters setting (ie. C<lazycounter> parameter has no effect).
