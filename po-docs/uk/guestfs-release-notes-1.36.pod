...
C<@VAR@> substitutions in C<./run> are now fully quoted.  This is necessary so that (eg) C<./configure PYTHON=/some/path> works robustly if C</some/path> contains characters that need to be quoted (Hilko Bengen).
