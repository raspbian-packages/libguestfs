...
namespace Guestfs
{
  ...
  class Guestfs
  {
    ...
    [DllImport ("libguestfs.so.0")]
    static extern int guestfs_inspect_get_windows_group_policy (IntPtr h, [In] string root);

    /// <summary>
    /// return if Windows guest has group policy
    /// </summary>
    public bool inspect_get_windows_group_policy (string root)
    {
      int r;
      r = guestfs_inspect_get_windows_group_policy (_handle, root);
      if (r == -1)
        throw new Error (guestfs_last_error (_handle));
      return r != 0 ? true : false;
    }
    ...
    [DllImport ("libguestfs.so.0")]
    static extern string guestfs_luks_uuid (IntPtr h, [In] string device);
    ...
    {
      string r;
      r = guestfs_luks_uuid (_handle, device);
      if (r == null)
        throw new Error (guestfs_last_error (_handle));
      return r;
    }
    ...
    [DllImport ("libguestfs.so.0")]
    static extern int guestfs_ntfs_chmod_argv (IntPtr h, [In] string device, int mode, [In] string path, void *);
    ...
    {
      int r;
      r = guestfs_ntfs_chmod_argv (_handle, device, mode, path, NULL);
      if (r == -1)
        throw new Error (guestfs_last_error (_handle));
    }
    ...
    {
      string r;
      r = guestfs_part_get_gpt_guid (_handle, device, partnum);
      if (r == null)
        throw new Error (guestfs_last_error (_handle));
      return r;
    }
    ...
    [DllImport ("libguestfs.so.0")]
    static extern int guestfs_part_set_gpt_guid (IntPtr h, [In] string device, int partnum, [In] string guid);
    ...
    {
      int r;
      r = guestfs_part_set_gpt_guid (_handle, device, partnum, guid);
      if (r == -1)
        throw new Error (guestfs_last_error (_handle));
    }
    ...
    [DllImport ("libguestfs.so.0")]
    static extern int guestfs_setfiles_argv (IntPtr h, [In] string specfile, [In] string[] paths, void *);
    ...
    {
      int r;
      r = guestfs_setfiles_argv (_handle, specfile, paths, NULL);
      if (r == -1)
        throw new Error (guestfs_last_error (_handle));
    }
