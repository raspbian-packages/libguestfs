...
=item $hasgrouppolicy = $g->inspect_get_windows_group_policy ($root);

This returns true if the Windows guest
...
=item $g->ntfs_chmod ($device, $mode, $path [, recursive => $recursive]);

Change file permissions on an NTFS filesystem with
...
I<This function is deprecated.>
In new code, use the L</setfiles> call instead.
...
=item $g->setfiles ($specfile, \@paths [, force => $force]);
