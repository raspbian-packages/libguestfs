 ...
 * @512 (the default if the parameter is omitted) or @4096. Use @4096
...
guestfs_session_add_drive (GuestfsSession *session, const gchar *filename, GuestfsAddDrive *optargs, GError **err)
{
  ...
  if (optargs) {
    ...
    if (blocksize != -1) {
      argv.bitmask |= GUESTFS_ADD_DRIVE_OPTS_BLOCKSIZE_BITMASK;
...
guestfs_session_add_drive_scratch (GuestfsSession *session, gint64 size, GuestfsAddDriveScratch *optargs, GError **err)
{
  ...
  if (optargs) {
    ...
    gint32 blocksize = g_value_get_int (&blocksize_v);
    if (blocksize != -1) {
      argv.bitmask |= GUESTFS_ADD_DRIVE_SCRATCH_BLOCKSIZE_BITMASK;
      argv.blocksize = blocksize;
    }
    argvp = &argv;
  }
...
/**
 * guestfs_session_inspect_get_windows_group_policy:
 * @session: (transfer none): A GuestfsSession object
 * @root: (transfer none) (type filename):
 * @err: A GError object to receive any generated errors
 *
 ...
 *
 * This returns true if the Windows guest has group policy. Group policy
 * can interfere with device driver installation, preventing libguestfs
 * features like driver injection from working.
 * 
 * Note that the presence of group policy is only an indication that there
 * may be a problem with driver installation. As group policy is extremely
 * complex and covers many different aspects of Windows configuration, and
 * we make no attempt to parse it, presence of group policy should only
 * raise a warning.
 * 
 * This call assumes that the guest is Windows and that the Registry could
 * be examined by inspection. If this is not the case then an error is
 ...
 */
gint8
guestfs_session_inspect_get_windows_group_policy (GuestfsSession *session, const gchar *root, GError **err)
{
  guestfs_h *g = session->priv->g;
  if (g == NULL) {
    g_set_error (err, GUESTFS_ERROR, 0,
  ...
  int ret = guestfs_inspect_get_windows_group_policy (g, root);
  if (ret == -1) {
    g_set_error_literal (err, GUESTFS_ERROR, 0, guestfs_last_error (g));
    return -1;
  }
 ...
 * 
 * @app2_class
 * Classification or primary use of this application. Currently the
 * only defined classification is "antivirus" for Windows antivirus
 * software. If unknown this is returned as an empty string "".
 * 
 * Please read "INSPECTION" in guestfs(3) for more details.
 * 
...
guestfs_session_inspect_list_applications2 (GuestfsSession *session, const gchar *root, GError **err)
{
  ...
  for (i = 0; i < ret->len; i++) {
    ...
    if (ret->val[i].app2_class) l[i]->app2_class = g_strdup (ret->val[i].app2_class);
 ...
 */
gchar *
guestfs_session_luks_uuid (GuestfsSession *session, const gchar *device, GError **err)
{
  guestfs_h *g = session->priv->g;
  if (g == NULL) {
    g_set_error (err, GUESTFS_ERROR, 0,
  ...
  char *ret = guestfs_luks_uuid (g, device);
  if (ret == NULL) {
    g_set_error_literal (err, GUESTFS_ERROR, 0, guestfs_last_error (g));
    return NULL;
  }
 ...
 * @10. Some of these are synonymous, and more levels may be added in
 * future.
 * 
...
/**
 * guestfs_session_ntfs_chmod:
 * @session: (transfer none): A GuestfsSession object
 * @device: (transfer none) (type filename):
 * @mode: (type gint32):
 * @path: (transfer none) (type utf8):
 * @optargs: (transfer none) (allow-none): a GuestfsNtfsChmod containing optional arguments
 * @err: A GError object to receive any generated errors
 *
 * change file permissions on NTFS filesystem
 *
 * Change file permissions on an NTFS filesystem with a chmod-style
 * permission mask, using ntfssecaudit(8). Only numeric modes are
 * supported.
 * 
 * *Note*: When using this command from guestfish, @mode by default would
 * be decimal, unless you prefix it with @0 to get octal, ie. use @0700 not
 * @700.
 * 
 * The @device parameter is the NTFS partition device (eg. "/dev/sda5").
 * This must be unmounted. The @mode parameter is the permission mode (eg.
 * @0744). The @path parameter is the file or directory path within the
 * filesystem. If the optional @recursive flag is set, directory
 ...
 */
gboolean
guestfs_session_ntfs_chmod (GuestfsSession *session, const gchar *device, gint32 mode, const gchar *path, GuestfsNtfsChmod *optargs, GError **err)
{
  guestfs_h *g = session->priv->g;
  if (g == NULL) {
    g_set_error (err, GUESTFS_ERROR, 0,
  ...
  struct guestfs_ntfs_chmod_argv argv;
  struct guestfs_ntfs_chmod_argv *argvp = NULL;

  if (optargs) {
    argv.bitmask = 0;

    GValue recursive_v = {0, };
    g_value_init (&recursive_v, GUESTFS_TYPE_TRISTATE);
    g_object_get_property (G_OBJECT (optargs), "recursive", &recursive_v);
    GuestfsTristate recursive = g_value_get_enum (&recursive_v);
    if (recursive != GUESTFS_TRISTATE_NONE) {
      argv.bitmask |= GUESTFS_NTFS_CHMOD_RECURSIVE_BITMASK;
      argv.recursive = recursive;
    }
    argvp = &argv;
  }
  int ret = guestfs_ntfs_chmod_argv (g, device, mode, path, argvp);
  if (ret == -1) {
    g_set_error_literal (err, GUESTFS_ERROR, 0, guestfs_last_error (g));
    return FALSE;
  }
 ...
 * read a file
 *
 * This call returns the contents of the file @path as a buffer.
 * 
 ...
 *
 * Returns: true on success, false on error
 * Deprecated: In new code, use guestfs_session_setfiles() instead
 * Since: 1.33.43
 */
gboolean
...
/**
 * guestfs_session_setfiles:
 * @session: (transfer none): A GuestfsSession object
 * @specfile: (transfer none) (type utf8):
 * @paths: (transfer none) (array zero-terminated=1) (element-type filename): an array of strings
 * @optargs: (transfer none) (allow-none): a GuestfsSetfiles containing optional arguments
 * @err: A GError object to receive any generated errors
 *
 * low level relabel parts of the filesystem
 *
 * This invokes the SELinux @setfiles command which is a low level tool
 * used to relabel parts of the filesystem.
 * 
 * The @specfile parameter controls the policy spec file used. You have to
 * parse "/etc/selinux/config" to find the correct SELinux policy and then
 * pass the spec file, usually: "/etc/selinux/" + *selinuxtype* +
 * "/contexts/files/file_contexts".
 * 
 * The required @paths parameter is the list of top level directories where
 * relabelling starts. @setfiles will only relabel up to filesystem
 * boundaries so, for example, passing just "/" will relabel the whole root
 * filesystem, but no other mounted filesystems. If the list is empty,
 * setfiles is not called.
 * 
 * The optional @force boolean controls whether the context is reset for
 * customizable files, and also whether the user, role and range parts of
 * the file context is changed.
 ...
 */
gboolean
guestfs_session_setfiles (GuestfsSession *session, const gchar *specfile, gchar *const *paths, GuestfsSetfiles *optargs, GError **err)
{
  guestfs_h *g = session->priv->g;
  if (g == NULL) {
    g_set_error (err, GUESTFS_ERROR, 0,
  ...
  struct guestfs_setfiles_argv argv;
  struct guestfs_setfiles_argv *argvp = NULL;
  ...
  if (optargs) {
    ...
    GuestfsTristate force = g_value_get_enum (&force_v);
    if (force != GUESTFS_TRISTATE_NONE) {
      argv.bitmask |= GUESTFS_SETFILES_FORCE_BITMASK;
      argv.force = force;
    }
    argvp = &argv;
  }
  int ret = guestfs_setfiles_argv (g, specfile, paths, argvp);
  if (ret == -1) {
    g_set_error_literal (err, GUESTFS_ERROR, 0, guestfs_last_error (g));
    return FALSE;
  }
