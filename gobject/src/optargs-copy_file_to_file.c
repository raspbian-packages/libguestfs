...
G_DEFINE_TYPE_WITH_CODE (GuestfsCopyFileToFile, guestfs_copy_file_to_file, G_TYPE_OBJECT,
                         G_ADD_PRIVATE (GuestfsCopyFileToFile));
...
enum {
  ...
  PROP_GUESTFS_COPY_FILE_TO_FILE_APPEND
};
...
guestfs_copy_file_to_file_set_property(GObject *object, guint property_id, const GValue *value, GParamSpec *pspec)
{
  ...
  switch (property_id) {
    ...
    case PROP_GUESTFS_COPY_FILE_TO_FILE_APPEND:
...
{
  o->priv = guestfs_copy_file_to_file_get_instance_private (o);
