    ...
    fn try_from(raw: *const RawApplication2) -> Result<Self, Self::Error> {
        Ok(unsafe {
            Application2 {
                ...
                app2_class: char_ptr_to_string((*raw).app2_class)?,
...
/* Optional Structs */
#[derive(Default)]
pub struct NtfsChmodOptArgs {
    pub recursive: Option<bool>,
}

struct CExprNtfsChmodOptArgs {
    recursive: Option<c_int>,
}

impl TryFrom<NtfsChmodOptArgs> for CExprNtfsChmodOptArgs {
    type Error = Error;
    fn try_from(optargs: NtfsChmodOptArgs) -> Result<Self, Self::Error> {
        Ok(CExprNtfsChmodOptArgs {
        recursive: optargs.recursive.map(|b| if b { 1 } else { 0 }),
         })
    }
}
#[repr(C)]
struct RawNtfsChmodOptArgs {
    bitmask: u64,
    recursive: c_int,
}

impl convert::From<&CExprNtfsChmodOptArgs> for RawNtfsChmodOptArgs {
    fn from(optargs: &CExprNtfsChmodOptArgs) -> Self {
        let mut bitmask = 0;
        RawNtfsChmodOptArgs {
        recursive: if let Some(v) = optargs.recursive {
            bitmask |= 1 << 0;
            v
        } else {
            0
...
/* Optional Structs */
#[derive(Default)]
pub struct SetfilesOptArgs {
    pub force: Option<bool>,
}

struct CExprSetfilesOptArgs {
    force: Option<c_int>,
}

impl TryFrom<SetfilesOptArgs> for CExprSetfilesOptArgs {
    type Error = Error;
    fn try_from(optargs: SetfilesOptArgs) -> Result<Self, Self::Error> {
        Ok(CExprSetfilesOptArgs {
        force: optargs.force.map(|b| if b { 1 } else { 0 }),
         })
    }
}
#[repr(C)]
struct RawSetfilesOptArgs {
    bitmask: u64,
    force: c_int,
}

impl convert::From<&CExprSetfilesOptArgs> for RawSetfilesOptArgs {
    fn from(optargs: &CExprSetfilesOptArgs) -> Self {
        let mut bitmask = 0;
        RawSetfilesOptArgs {
        force: if let Some(v) = optargs.force {
            bitmask |= 1 << 0;
...
extern "C" {
    ...
    #[allow(non_snake_case)]
    fn guestfs_inspect_get_windows_group_policy(g: *const guestfs_h, root: *const c_char) -> c_int;
    #[allow(non_snake_case)]
    ...
    #[allow(non_snake_case)]
    fn guestfs_ntfs_chmod_argv(g: *const guestfs_h, device: *const c_char, mode: c_int, path: *const c_char, optarg: *const RawNtfsChmodOptArgs) -> c_int;
    #[allow(non_snake_case)]
    ...
    #[allow(non_snake_case)]
    fn guestfs_setfiles_argv(g: *const guestfs_h, specfile: *const c_char, paths: *const *const c_char, optarg: *const RawSetfilesOptArgs) -> c_int;
    #[allow(non_snake_case)]
...
, (&c_val).as_ptr()) };
    ...
    #[allow(non_snake_case)]
    pub fn inspect_get_windows_group_policy(&self, root: &str) -> Result<bool, Error> {
        let c_root = ffi::CString::new(root)?;
        
        let r = unsafe { guestfs_inspect_get_windows_group_policy(self.g, (&c_root).as_ptr()) };
        if r == -1 {
            return Err(self.get_error_from_handle("inspect_get_windows_group_policy"));
        }
        drop(c_root);
        Ok(r != 0)
    }
...
, (&c_strlist).as_ptr() as *const *const c_char, b, integer, integer64, (&c_filein).as_ptr(), (&c_fileout).as_ptr(), (&c_bufferin).as_ptr(), c_bufferin_len, &(RawInternalTestOptArgs::from(&optargs_cexpr)) as *const RawInternalTestOptArgs) };
    ...
    #[allow(non_snake_case)]
    pub fn ntfs_chmod(&self, device: &str, mode: i32, path: &str, optargs: NtfsChmodOptArgs) -> Result<(), Error> {
        let c_device = ffi::CString::new(device)?;
        let c_path = ffi::CString::new(path)?;
        let optargs_cexpr = CExprNtfsChmodOptArgs::try_from(optargs)?;
        
        let r = unsafe { guestfs_ntfs_chmod_argv(self.g, (&c_device).as_ptr(), mode, (&c_path).as_ptr(), &(RawNtfsChmodOptArgs::from(&optargs_cexpr)) as *const RawNtfsChmodOptArgs) };
        if r == -1 {
            return Err(self.get_error_from_handle("ntfs_chmod"));
        }
        drop(c_device);
        drop(c_path);
        drop(optargs_cexpr);
    ...
    #[allow(non_snake_case)]
    pub fn setfiles(&self, specfile: &str, paths: &[&str], optargs: SetfilesOptArgs) -> Result<(), Error> {
        let c_specfile = ffi::CString::new(specfile)?;
        let c_paths_v = arg_string_list(paths)?;
        let mut c_paths = (&c_paths_v).into_iter().map(|v| v.as_ptr()).collect::<Vec<_>>();
        c_paths.push(ptr::null());
        let optargs_cexpr = CExprSetfilesOptArgs::try_from(optargs)?;
        
        let r = unsafe { guestfs_setfiles_argv(self.g, (&c_specfile).as_ptr(), (&c_paths).as_ptr() as *const *const c_char, &(RawSetfilesOptArgs::from(&optargs_cexpr)) as *const RawSetfilesOptArgs) };
        if r == -1 {
            return Err(self.get_error_from_handle("setfiles"));
        }
        drop(c_specfile);
        drop(c_paths);
        drop(c_paths_v);
