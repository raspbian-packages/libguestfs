...
#define CLEANUP_XDR_FREE_FINDFS_PARTUUID_ARGS \
    __attribute__((cleanup(cleanup_xdr_free_findfs_partuuid_args)))

static void
cleanup_xdr_free_findfs_partuuid_args (struct guestfs_findfs_partuuid_args *argsp)
{
  xdr_free ((xdrproc_t) xdr_guestfs_findfs_partuuid_args, (char *) argsp);
}


void
findfs_partuuid_stub (XDR *xdr_in)
{
  CLEANUP_FREE char *r = NULL;
  CLEANUP_XDR_FREE_FINDFS_PARTUUID_ARGS struct guestfs_findfs_partuuid_args args;
  memset (&args, 0, sizeof args);
  const char *uuid;
  ...
  if (!xdr_guestfs_findfs_partuuid_args (xdr_in, &args)) {
    reply_with_error ("daemon failed to decode procedure arguments");
    return;
  }
  uuid = args.uuid;

  r = do_findfs_partuuid (uuid);
  if (r == NULL)
    /* do_findfs_partuuid has already called reply_with_error */
    return;
  ...
  if (rr == NULL)
    ...
    return;
  ret.device = rr;
  reply ((xdrproc_t) &xdr_guestfs_findfs_partuuid_ret, (char *) &ret);
}
...
#define CLEANUP_XDR_FREE_SETFILES_ARGS \
    __attribute__((cleanup(cleanup_xdr_free_setfiles_args)))

static void
cleanup_xdr_free_setfiles_args (struct guestfs_setfiles_args *argsp)
{
  xdr_free ((xdrproc_t) xdr_guestfs_setfiles_args, (char *) argsp);
}


void
setfiles_stub (XDR *xdr_in)
{
  int r;
  CLEANUP_XDR_FREE_SETFILES_ARGS struct guestfs_setfiles_args args;
  memset (&args, 0, sizeof args);
  const char *specfile;
  char **paths;
  int force;
  ...
  if (!xdr_guestfs_setfiles_args (xdr_in, &args)) {
    reply_with_error ("daemon failed to decode procedure arguments");
    return;
  }
  ...
  {
    size_t i;
    for (i = 0; i < args.paths.paths_len; ++i) {
      ABS_PATH (args.paths.paths_val[i], false, return);
    }
  }
  /* Ugly, but safe and avoids copying the strings. */
  paths = realloc (args.paths.paths_val,
                sizeof (char *) * (args.paths.paths_len+1));
  if (paths == NULL) {
    reply_with_perror ("realloc");
    return;
  }
  paths[args.paths.paths_len] = NULL;
  args.paths.paths_val = paths;
  force = args.force;

  r = do_setfiles (specfile, paths, force);
  if (r == -1)
    /* do_setfiles has already called reply_with_error */
    return;
