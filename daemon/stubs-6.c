...
cryptsetup_open_stub (XDR *xdr_in)
{
  ...
  r = do_cryptsetup_open (device, key, mapname, readonly, crypttype, cipher);
  if (r == -1)
...
#define CLEANUP_XDR_FREE_INSPECT_GET_WINDOWS_GROUP_POLICY_ARGS \
    __attribute__((cleanup(cleanup_xdr_free_inspect_get_windows_group_policy_args)))

static void
cleanup_xdr_free_inspect_get_windows_group_policy_args (struct guestfs_inspect_get_windows_group_policy_args *argsp)
{
  xdr_free ((xdrproc_t) xdr_guestfs_inspect_get_windows_group_policy_args, (char *) argsp);
}


void
inspect_get_windows_group_policy_stub (XDR *xdr_in)
{
  int r;
  CLEANUP_XDR_FREE_INSPECT_GET_WINDOWS_GROUP_POLICY_ARGS struct guestfs_inspect_get_windows_group_policy_args args;
  memset (&args, 0, sizeof args);
  CLEANUP_FREE_MOUNTABLE mountable_t root
      = { .device = NULL, .volume = NULL };
  ...
  if (!xdr_guestfs_inspect_get_windows_group_policy_args (xdr_in, &args)) {
    reply_with_error ("daemon failed to decode procedure arguments");
    return;
  }
  RESOLVE_MOUNTABLE (args.root, root, false);

  r = do_inspect_get_windows_group_policy (&root);
  if (r == -1)
    /* do_inspect_get_windows_group_policy has already called reply_with_error */
    return;

  struct guestfs_inspect_get_windows_group_policy_ret ret;
  ret.hasgrouppolicy = r;
  reply ((xdrproc_t) &xdr_guestfs_inspect_get_windows_group_policy_ret, (char *) &ret);
}
