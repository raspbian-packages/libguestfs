...
#define GUESTFS_NTFS_CHMOD_RECURSIVE_BITMASK (UINT64_C(1)<<0)
...
#define GUESTFS_SETFILES_FORCE_BITMASK (UINT64_C(1)<<0)
...
extern int do_aug_transform (const char *lens, const char *file, int remove);
...
extern char *do_file_architecture (const char *filename);
...
extern int do_hivex_open (const char *filename, int verbose, int debug, int write, int unsafe);
extern int64_t do_hivex_root (void);
extern char *do_hivex_value_key (int64_t valueh);
extern char *do_hivex_value_string (int64_t valueh);
extern int64_t do_hivex_value_type (int64_t valueh);
extern char *do_hivex_value_utf8 (int64_t valueh);
...
extern char *do_inspect_get_format (const mountable_t *root);
extern char *do_inspect_get_hostname (const mountable_t *root);
extern int do_inspect_get_major_version (const mountable_t *root);
extern int do_inspect_get_minor_version (const mountable_t *root);
extern char **do_inspect_get_mountpoints (const mountable_t *root);
extern char *do_inspect_get_package_format (const mountable_t *root);
extern char *do_inspect_get_package_management (const mountable_t *root);
extern char *do_inspect_get_product_name (const mountable_t *root);
extern char *do_inspect_get_product_variant (const mountable_t *root);
extern char **do_inspect_get_roots (void);
extern char *do_inspect_get_type (const mountable_t *root);
extern char *do_inspect_get_windows_current_control_set (const mountable_t *root);
extern int do_inspect_get_windows_group_policy (const mountable_t *root);
extern char *do_inspect_get_windows_software_hive (const mountable_t *root);
extern char *do_inspect_get_windows_system_hive (const mountable_t *root);
extern char *do_inspect_get_windows_systemroot (const mountable_t *root);
extern int do_inspect_is_live (const mountable_t *root);
extern int do_inspect_is_multipart (const mountable_t *root);
extern int do_inspect_is_netinst (const mountable_t *root);
extern char **do_inspect_os (void);
...
extern int do_internal_find_inode (const mountable_t *device, int64_t inode);
...
extern int do_internal_yara_scan (const char *path);
...
extern char **do_list_filesystems (void);
...
extern char *do_luks_uuid (const char *device);
...
extern int do_mksquashfs (const char *path, const char *compress, char *const *excludes);
...
extern int do_ntfs_chmod (const char *device, int mode, const char *path, int recursive);
...
extern int64_t do_part_get_gpt_attributes (const char *device, int partnum);
...
extern int do_part_resize (const char *device, int partnum, int64_t endsect);
...
extern int do_part_set_gpt_attributes (const char *device, int partnum, int64_t attributes);
...
extern int do_setfiles (const char *specfile, char *const *paths, int force);
...
extern int do_yara_destroy (void);
extern int do_yara_load (void);
