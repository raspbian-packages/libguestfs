...
#define OPTGROUP_LUKS_NOT_AVAILABLE \
  ...
  char *__attribute__((noreturn)) do_luks_uuid (const char *device) { abort (); } \
...
#define OPTGROUP_NTFS3G_NOT_AVAILABLE \
  ...
  int __attribute__((noreturn)) do_ntfs_chmod (const char *device, int mode, const char *path, int recursive) { abort (); } \
...
#define OPTGROUP_SELINUXRELABEL_NOT_AVAILABLE \
  ...
  int __attribute__((noreturn)) do_setfiles (const char *specfile, char *const *paths, int force) { abort (); } \
