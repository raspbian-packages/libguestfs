...
return_application2 (value retv)
{
  ...
  v = Field (retv, 13);
  ret->app2_class = strdup (String_val (v));
  if (ret->app2_class == NULL) return NULL;
  v = Field (retv, 14);
...
/* Wrapper for OCaml function ‘Inspect.inspect_get_windows_group_policy’. */
int
do_inspect_get_windows_group_policy (const mountable_t *root)
{
  static const value *cb = NULL;
  CAMLparam0 ();
  CAMLlocal2 (v, retv);
  ...
  if (cb == NULL)
    cb = caml_named_value ("Inspect.inspect_get_windows_group_policy");
  ...
  if (Is_exception_result (retv)) {
    retv = Extract_exception (retv);
    guestfs_int_daemon_exn_to_reply_with_error ("inspect_get_windows_group_policy", retv);
    CAMLreturnT (int, -1);
  }
...
/* Wrapper for OCaml function ‘Selinux.selinux_relabel’. */
int
do_selinux_relabel (const char *specfile,
                    const char *path,
                    int force)
{
  static const value *cb = NULL;
  ...
  if (cb == NULL)
    cb = caml_named_value ("Selinux.selinux_relabel");

  if ((optargs_bitmask & GUESTFS_SELINUX_RELABEL_FORCE_BITMASK) == 0)
    args[0] = Val_int (0); /* None */
  else {
    v = Val_bool (force);
    args[0] = caml_alloc (1, 0);
  ...
  args[1] = caml_copy_string (specfile);
  args[2] = caml_copy_string (path);
  retv = caml_callbackN_exn (*cb, 3, args);

  if (Is_exception_result (retv)) {
    retv = Extract_exception (retv);
    guestfs_int_daemon_exn_to_reply_with_error ("selinux_relabel", retv);
    CAMLreturnT (int, -1);
  }
...
/* Wrapper for OCaml function ‘Selinux.setfiles’. */
int
do_setfiles (const char *specfile,
             char *const *paths,
             int force)
{
  static const value *cb = NULL;
  ...
  if (cb == NULL)
    cb = caml_named_value ("Selinux.setfiles");

  if ((optargs_bitmask & GUESTFS_SETFILES_FORCE_BITMASK) == 0)
    args[0] = Val_int (0); /* None */
  else {
    v = Val_bool (force);
    args[0] = caml_alloc (1, 0);
  ...
  args[1] = caml_copy_string (specfile);
  args[2] = guestfs_int_daemon_copy_string_list (paths);
  retv = caml_callbackN_exn (*cb, 3, args);

  if (Is_exception_result (retv)) {
    retv = Extract_exception (retv);
    guestfs_int_daemon_exn_to_reply_with_error ("setfiles", retv);
    CAMLreturnT (int, -1);
  }
