                           ...
                           va_list args)
{
  ...
  while ((i = va_arg (args, int)) >= 0) {
    ...
    case GUESTFS_ADD_DRIVE_OPTS_COPYONREAD:
      optargs_s.copyonread = va_arg (args, int);
      break;
    case GUESTFS_ADD_DRIVE_OPTS_BLOCKSIZE:
                              ...
                              va_list args)
{
  ...
  while ((i = va_arg (args, int)) >= 0) {
    ...
    case GUESTFS_ADD_DRIVE_SCRATCH_LABEL:
      optargs_s.label = va_arg (args, const char *);
      break;
    case GUESTFS_ADD_DRIVE_SCRATCH_BLOCKSIZE:
      optargs_s.blocksize = va_arg (args, int);
      break;
    default:
      error (g, "%s: unknown option %d (this can happen if a program is compiled against a newer version of libguestfs, then dynamically linked to an older version)",
...
int
guestfs_ntfs_chmod (guestfs_h *g,
                    const char *device,
                    int mode,
                    const char *path,
                    ...)
{
  ...
  va_start (optargs, path);
  r = guestfs_ntfs_chmod_va (g, device, mode, path, optargs);
  va_end (optargs);
...
int
guestfs_ntfs_chmod_va (guestfs_h *g,
                       const char *device,
                       int mode,
                       const char *path,
                       va_list args)
{
  ...
  while ((i = va_arg (args, int)) >= 0) {
    switch (i) {
    case GUESTFS_NTFS_CHMOD_RECURSIVE:
      optargs_s.recursive = va_arg (args, int);
      break;
    default:
      error (g, "%s: unknown option %d (this can happen if a program is compiled against a newer version of libguestfs, then dynamically linked to an older version)",
             "ntfs_chmod", i);
  ...
  return guestfs_ntfs_chmod_argv (g, device, mode, path, optargs);
}
                  ...
                  ...)
{
  ...
  va_start (optargs, paths);
  r = guestfs_setfiles_va (g, specfile, paths, optargs);
  va_end (optargs);
...
int
guestfs_setfiles_va (guestfs_h *g,
                     const char *specfile,
                     char *const *paths,
                     va_list args)
{
  ACQUIRE_LOCK_FOR_CURRENT_SCOPE (&g->lock);
  struct guestfs_setfiles_argv optargs_s;
  ...
  return guestfs_setfiles_argv (g, specfile, paths, optargs);
}
