...
{
  ACQUIRE_LOCK_FOR_CURRENT_SCOPE (&g->lock);
  struct guestfs_findfs_partuuid_args args;
  guestfs_message_header hdr;
  guestfs_message_error err;
  struct guestfs_findfs_partuuid_ret ret;
  int serial;
  int r;
  int trace_flag = g->trace;
  struct trace_buffer trace_buffer;
  ...
  if (uuid == NULL) {
    error (g, "%s: %s: parameter cannot be NULL",
           "findfs_partuuid", "uuid");
    return NULL;
  }

  if (trace_flag) {
    guestfs_int_trace_open (&trace_buffer);
    fprintf (trace_buffer.fp, "%s", "findfs_partuuid");
    fprintf (trace_buffer.fp, " \"%s\"", uuid);
    guestfs_int_trace_send_line (g, &trace_buffer);
  }

  if (guestfs_int_check_appliance_up (g, "findfs_partuuid") == -1) {
    if (trace_flag)
      guestfs_int_trace (g, "%s = %s (error)",
                         "findfs_partuuid", "NULL");
    return NULL;
  }

  args.uuid = (char *) uuid;
  serial = guestfs_int_send (g, GUESTFS_PROC_FINDFS_PARTUUID,
                             progress_hint, 0,
                             (xdrproc_t) xdr_guestfs_findfs_partuuid_args, (char *) &args);
  if (serial == -1) {
    if (trace_flag)
      guestfs_int_trace (g, "%s = %s (error)",
                         "findfs_partuuid", "NULL");
  ...
  r = guestfs_int_recv (g, "findfs_partuuid", &hdr, &err,
        (xdrproc_t) xdr_guestfs_findfs_partuuid_ret, (char *) &ret);
  if (r == -1) {
    if (trace_flag)
      guestfs_int_trace (g, "%s = %s (error)",
                         "findfs_partuuid", "NULL");
  ...
  if (guestfs_int_check_reply_header (g, &hdr, GUESTFS_PROC_FINDFS_PARTUUID, serial) == -1) {
    if (trace_flag)
      guestfs_int_trace (g, "%s = %s (error)",
                         "findfs_partuuid", "NULL");
    return NULL;
  ...
  if (hdr.status == GUESTFS_STATUS_ERROR) {
    ...
    if (err.errno_string[0] != '\0')
      errnum = guestfs_int_string_to_errno (err.errno_string);
    if (errnum <= 0)
      error (g, "%s: %s", "findfs_partuuid", err.error_message);
    else
      guestfs_int_error_errno (g, errnum, "%s: %s", "findfs_partuuid",
                               err.error_message);
    free (err.error_message);
    free (err.errno_string);
    return NULL;
...
{
  ACQUIRE_LOCK_FOR_CURRENT_SCOPE (&g->lock);
  struct guestfs_inspect_get_format_args args;
  guestfs_message_header hdr;
  guestfs_message_error err;
  ...
  if (guestfs_int_check_appliance_up (g, "inspect_get_format") == -1) {
    if (trace_flag)
      guestfs_int_trace (g, "%s = %s (error)",
  ...
  args.root = (char *) root;
  serial = guestfs_int_send (g, GUESTFS_PROC_INSPECT_GET_FORMAT,
                             progress_hint, 0,
                             (xdrproc_t) xdr_guestfs_inspect_get_format_args, (char *) &args);
  if (serial == -1) {
    if (trace_flag)
      guestfs_int_trace (g, "%s = %s (error)",
                         "inspect_get_format", "NULL");
  ...
  r = guestfs_int_recv (g, "inspect_get_format", &hdr, &err,
        (xdrproc_t) xdr_guestfs_inspect_get_format_ret, (char *) &ret);
  if (r == -1) {
    if (trace_flag)
      guestfs_int_trace (g, "%s = %s (error)",
                         "inspect_get_format", "NULL");
  ...
  if (guestfs_int_check_reply_header (g, &hdr, GUESTFS_PROC_INSPECT_GET_FORMAT, serial) == -1) {
    if (trace_flag)
      guestfs_int_trace (g, "%s = %s (error)",
                         "inspect_get_format", "NULL");
    return NULL;
  ...
  if (hdr.status == GUESTFS_STATUS_ERROR) {
    ...
    if (err.errno_string[0] != '\0')
      errnum = guestfs_int_string_to_errno (err.errno_string);
    if (errnum <= 0)
      error (g, "%s: %s", "inspect_get_format", err.error_message);
    else
      guestfs_int_error_errno (g, errnum, "%s: %s", "inspect_get_format",
                               err.error_message);
    free (err.error_message);
    free (err.errno_string);
    return NULL;
  ...
  ret_v = ret.format; /* caller will free */
  if (trace_flag) {
    guestfs_int_trace_open (&trace_buffer);
    fprintf (trace_buffer.fp, "%s = ", "inspect_get_format");
    fprintf (trace_buffer.fp, "\"%s\"", ret_v);
...
{
  ACQUIRE_LOCK_FOR_CURRENT_SCOPE (&g->lock);
  struct guestfs_inspect_get_hostname_args args;
  guestfs_message_header hdr;
  guestfs_message_error err;
  ...
  if (guestfs_int_check_appliance_up (g, "inspect_get_hostname") == -1) {
    if (trace_flag)
      guestfs_int_trace (g, "%s = %s (error)",
  ...
  args.root = (char *) root;
  serial = guestfs_int_send (g, GUESTFS_PROC_INSPECT_GET_HOSTNAME,
                             progress_hint, 0,
                             (xdrproc_t) xdr_guestfs_inspect_get_hostname_args, (char *) &args);
  if (serial == -1) {
    if (trace_flag)
      guestfs_int_trace (g, "%s = %s (error)",
                         "inspect_get_hostname", "NULL");
  ...
  r = guestfs_int_recv (g, "inspect_get_hostname", &hdr, &err,
        (xdrproc_t) xdr_guestfs_inspect_get_hostname_ret, (char *) &ret);
  if (r == -1) {
    if (trace_flag)
      guestfs_int_trace (g, "%s = %s (error)",
                         "inspect_get_hostname", "NULL");
  ...
  if (guestfs_int_check_reply_header (g, &hdr, GUESTFS_PROC_INSPECT_GET_HOSTNAME, serial) == -1) {
    if (trace_flag)
      guestfs_int_trace (g, "%s = %s (error)",
                         "inspect_get_hostname", "NULL");
    return NULL;
  ...
  if (hdr.status == GUESTFS_STATUS_ERROR) {
    ...
    if (err.errno_string[0] != '\0')
      errnum = guestfs_int_string_to_errno (err.errno_string);
    if (errnum <= 0)
      error (g, "%s: %s", "inspect_get_hostname", err.error_message);
    else
      guestfs_int_error_errno (g, errnum, "%s: %s", "inspect_get_hostname",
                               err.error_message);
    free (err.error_message);
    free (err.errno_string);
    return NULL;
  ...
  ret_v = ret.hostname; /* caller will free */
  if (trace_flag) {
    guestfs_int_trace_open (&trace_buffer);
    fprintf (trace_buffer.fp, "%s = ", "inspect_get_hostname");
    fprintf (trace_buffer.fp, "\"%s\"", ret_v);
...
{
  ACQUIRE_LOCK_FOR_CURRENT_SCOPE (&g->lock);
  struct guestfs_inspect_is_multipart_args args;
  guestfs_message_header hdr;
  guestfs_message_error err;
  ...
  if (guestfs_int_check_appliance_up (g, "inspect_is_multipart") == -1) {
    if (trace_flag)
      guestfs_int_trace (g, "%s = %s (error)",
  ...
  args.root = (char *) root;
  serial = guestfs_int_send (g, GUESTFS_PROC_INSPECT_IS_MULTIPART,
                             progress_hint, 0,
                             (xdrproc_t) xdr_guestfs_inspect_is_multipart_args, (char *) &args);
  if (serial == -1) {
    if (trace_flag)
      guestfs_int_trace (g, "%s = %s (error)",
                         "inspect_is_multipart", "-1");
  ...
  r = guestfs_int_recv (g, "inspect_is_multipart", &hdr, &err,
        (xdrproc_t) xdr_guestfs_inspect_is_multipart_ret, (char *) &ret);
  if (r == -1) {
    if (trace_flag)
      guestfs_int_trace (g, "%s = %s (error)",
                         "inspect_is_multipart", "-1");
  ...
  if (guestfs_int_check_reply_header (g, &hdr, GUESTFS_PROC_INSPECT_IS_MULTIPART, serial) == -1) {
    if (trace_flag)
      guestfs_int_trace (g, "%s = %s (error)",
                         "inspect_is_multipart", "-1");
    return -1;
  ...
  if (hdr.status == GUESTFS_STATUS_ERROR) {
    ...
    if (err.errno_string[0] != '\0')
      errnum = guestfs_int_string_to_errno (err.errno_string);
    if (errnum <= 0)
      error (g, "%s: %s", "inspect_is_multipart", err.error_message);
    else
      guestfs_int_error_errno (g, errnum, "%s: %s", "inspect_is_multipart",
                               err.error_message);
    free (err.error_message);
    free (err.errno_string);
    return -1;
...
GUESTFS_DLL_PUBLIC int
guestfs_setfiles_argv (guestfs_h *g,
                       const char *specfile,
                       char *const *paths,
                       const struct guestfs_setfiles_argv *optargs)
{
  ACQUIRE_LOCK_FOR_CURRENT_SCOPE (&g->lock);
  struct guestfs_setfiles_argv optargs_null;
  if (!optargs) {
    optargs_null.bitmask = 0;
    optargs = &optargs_null;
  }

  struct guestfs_setfiles_args args;
  guestfs_message_header hdr;
  guestfs_message_error err;
  int serial;
  int r;
  ...
  if (trace_flag) {
    ...
    guestfs_int_trace_open (&trace_buffer);
    fprintf (trace_buffer.fp, "%s", "setfiles");
    fprintf (trace_buffer.fp, " \"%s\"", specfile);
    fputc (' ', trace_buffer.fp);
    fputc ('"', trace_buffer.fp);
    for (i = 0; paths[i]; ++i) {
      if (i > 0) fputc (' ', trace_buffer.fp);
      fputs (paths[i], trace_buffer.fp);
    }
    fputc ('"', trace_buffer.fp);
    if (optargs->bitmask & GUESTFS_SETFILES_FORCE_BITMASK) {
      fprintf (trace_buffer.fp, " \"%s:%s\"", "force", optargs->force ? "true" : "false");
    }
    guestfs_int_trace_send_line (g, &trace_buffer);
  }

  if (guestfs_int_check_appliance_up (g, "setfiles") == -1) {
    if (trace_flag)
      guestfs_int_trace (g, "%s = %s (error)",
                         "setfiles", "-1");
    return -1;
  ...
  args.paths.paths_val = (char **) paths;
  for (args.paths.paths_len = 0; paths[args.paths.paths_len]; args.paths.paths_len++) ;
  if (optargs->bitmask & GUESTFS_SETFILES_FORCE_BITMASK) {
    args.force = optargs->force;
  } else {
    args.force = 0;
  }
  serial = guestfs_int_send (g, GUESTFS_PROC_SETFILES,
                             progress_hint, optargs->bitmask,
                             (xdrproc_t) xdr_guestfs_setfiles_args, (char *) &args);
  if (serial == -1) {
    if (trace_flag)
      guestfs_int_trace (g, "%s = %s (error)",
                         "setfiles", "-1");
  ...
  r = guestfs_int_recv (g, "setfiles", &hdr, &err,
        NULL, NULL);
  if (r == -1) {
    if (trace_flag)
      guestfs_int_trace (g, "%s = %s (error)",
  ...
  if (guestfs_int_check_reply_header (g, &hdr, GUESTFS_PROC_SETFILES, serial) == -1) {
    if (trace_flag)
      guestfs_int_trace (g, "%s = %s (error)",
                         "setfiles", "-1");
    return -1;
  ...
  if (hdr.status == GUESTFS_STATUS_ERROR) {
    ...
    if (err.errno_string[0] != '\0')
      errnum = guestfs_int_string_to_errno (err.errno_string);
    if (errnum <= 0)
      error (g, "%s: %s", "setfiles", err.error_message);
    else
      guestfs_int_error_errno (g, errnum, "%s: %s", "setfiles",
                               err.error_message);
    free (err.error_message);
    free (err.errno_string);
    return -1;
...
GUESTFS_DLL_PUBLIC int
guestfs_sh_out (guestfs_h *g,
                const char *command,
                const char *output)
{
  ACQUIRE_LOCK_FOR_CURRENT_SCOPE (&g->lock);
  struct guestfs_sh_out_args args;
  guestfs_message_header hdr;
  guestfs_message_error err;
  int serial;
  int r;
  ...
  if (trace_flag) {
    guestfs_int_trace_open (&trace_buffer);
    fprintf (trace_buffer.fp, "%s", "sh_out");
    fprintf (trace_buffer.fp, " \"%s\"", command);
    fprintf (trace_buffer.fp, " \"%s\"", output);
    guestfs_int_trace_send_line (g, &trace_buffer);
  }

  if (guestfs_int_check_appliance_up (g, "sh_out") == -1) {
    if (trace_flag)
      guestfs_int_trace (g, "%s = %s (error)",
                         "sh_out", "-1");
    return -1;
  ...
  args.command = (char *) command;
  serial = guestfs_int_send (g, GUESTFS_PROC_SH_OUT,
                             progress_hint, 0,
                             (xdrproc_t) xdr_guestfs_sh_out_args, (char *) &args);
  if (serial == -1) {
    if (trace_flag)
      guestfs_int_trace (g, "%s = %s (error)",
                         "sh_out", "-1");
  ...
  r = guestfs_int_recv (g, "sh_out", &hdr, &err,
        NULL, NULL);
  if (r == -1) {
    if (trace_flag)
      guestfs_int_trace (g, "%s = %s (error)",
  ...
  if (guestfs_int_check_reply_header (g, &hdr, GUESTFS_PROC_SH_OUT, serial) == -1) {
    if (trace_flag)
      guestfs_int_trace (g, "%s = %s (error)",
                         "sh_out", "-1");
    return -1;
  ...
  if (hdr.status == GUESTFS_STATUS_ERROR) {
    ...
    if (err.errno_string[0] != '\0')
      errnum = guestfs_int_string_to_errno (err.errno_string);
    if (errnum <= 0)
      error (g, "%s: %s", "sh_out", err.error_message);
    else
      guestfs_int_error_errno (g, errnum, "%s: %s", "sh_out",
                               err.error_message);
    free (err.error_message);
    free (err.errno_string);
    return -1;
